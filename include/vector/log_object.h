 ...
 * for managing log objects used for buffered delta operations in vector indices.
 * Each log object consists of metadata and individual log items stored as
 * separate keys in the storage system.
 *
 * @author EloqData Inc.
 * @date 2025
 */
...
struct log_item_t
{
    ...
    log_item_t() = default;
    log_item_t(LogOperationType op, const std::string& k, const std::string& val, 
               uint64_t ts, uint64_t seq_id)
        : operation_type(op), key(k), value(val), timestamp(ts), sequence_id(seq_id) {}
};
...
struct log_metadata_t
{
    ...
    log_metadata_t() : list_count(0), head_item_sequence_id(0), tail_item_sequence_id(0), next_id(1) {}
};

/**
 * @brief Structure representing log statistics
 */
struct log_stats_t
{
    uint64_t total_items;
    uint64_t oldest_sequence_id;
    uint64_t newest_sequence_id;
    uint64_t total_size_bytes;
    
    log_stats_t() : total_items(0), oldest_sequence_id(0), newest_sequence_id(0), total_size_bytes(0) {}
};
...
public:
     ...
     */
    static LogError append_log(const std::string& log_name, std::vector<log_item_t>& items,
                              txservice::TransactionExecution* txm);
     ...
     */
    static LogError truncate_log(const std::string& log_name, uint64_t to_id,
                                txservice::TransactionExecution* txm);
     ...
     */
    static LogError scan_log(const std::string& log_name, uint64_t to_id, 
     ...
     * @param log_name Name of the log object
     * @param txm Transaction execution context (required, non-null)
     * @return log_stats_t containing log statistics
     */
    static log_stats_t get_stats(const std::string& log_name,
                                txservice::TransactionExecution* txm);
...
private:
     ...
     */
    static void serialize_metadata(const log_metadata_t& meta, std::string& result);
     ...
     */
    static log_metadata_t deserialize_metadata(const std::string& data);
     ...
     */
    static log_item_t deserialize_log_item(const std::string& data);
     ...
     */
    static std::string get_log_item_key(const std::string& log_name, uint64_t sequence_id);
